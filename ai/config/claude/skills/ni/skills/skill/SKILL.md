...
1. `claude plugin validate ./ai/config/claude/skills/ni` from the repo root
2. `claude --plugin-dir ./ai/config/claude/skills/ni -p "..."` to load from the working tree
3. Add the skill, agent, or command to the README table
4. Install: `rm -rf ~/.claude/skills && cp -r ai/config/claude/{skills,settings.json,statusline-command.sh} ~/.claude/`
