...
if [ "$level" != "off" ] && [ -z "$switched" ]; then
  case "$level" in
    full) rule="Drop articles, fragments allowed, shortest synonym. No filler, hedging, preamble, recap, or tool narration. Technical facts, code, commands, and errors exact." ;;
